    ...
    for (row = 0; row < kMaxRow; row++)
        for (corner = 0; corner < kMaxCorners; corner++)
        {
            ...
            for (bone = 0; bone < kMaxBones; bone++)
            {
                ...
                // Copy data to here to visualize your weights or anything else
                if (bone & 1)
                    g_boneWeightVis[row][corner].s += g_boneWeights[row][corner][bone];

                // Copy data to here to visualize your weights
                if ((bone+1) & 1)
                    g_boneWeightVis[row][corner].t += g_boneWeights[row][corner][bone];
            }
...
int main(int argc, char **argv)
{
    ...
    g_shader = loadShaders("skinning2.vert", "shader.frag");
